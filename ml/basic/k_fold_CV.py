...
for k in range(K+1):
    X_mat = np.matrix([X_mat,x**k])
    testX_mat = np.matrix([testX_mat,testx**k])
    tmpX_mat = X_mat
    tmpt_mat = t_mat
    for fold in range(K):
        trainX = del tmp_mat[fold*5:(fold+1)*5,:]
        foldX = X_mat[fold*5:(fold+1)*5,:]
        traint = del tmpt_mat[fold*5:(fold+1)*5,:]
        foldt = t_mat[fold*5:(fold+1)*5,:]

        w = (trainX.getT()*trainX).getI()*trainX.getT()*traint
        fold_pred = foldX*w
